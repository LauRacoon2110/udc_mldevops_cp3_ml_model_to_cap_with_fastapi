...
def test_save_model(mock_joblib_dump):
    """
...
def test_load_model(mock_joblib_load):
    """
...
def test_compute_model_metrics():
    ...
    with tempfile.NamedTemporaryFile(delete=True) as temp_file:
        precision, recall, f1 = compute_model_metrics(
            y_true, y_pred, output_file=temp_file.name  # Pass the temporary file's name
        )
...
def test_compute_model_metrics_on_slices_from_predictions(sample_dataframe):
    """
    ...
    with tempfile.NamedTemporaryFile(delete=True) as temp_file:
        results = compute_model_metrics_on_slices_from_predictions(
            ...
            output_file=temp_file.name,  # Pass the temporary file's name
        )
